...
void DisplayHandler(void)
{
    ...
    glEnable(GL_DEPTH_TEST);
    {
        ...
        (*(drawSkybox.cube_.shader_)).unbind();
    }
    ...
    if (objs.size()!=0)
    {
        const DrawObject& draw = drawObjs[objs.size()-1];
        const Object& obj = objs[objs.size()-1];
    ...
    for (int i =0;i<static_cast<int>(objs.size()-1);i++)
    {
        ...
        float mixfactor = inside? 1.0:0.2;
...
//     
    ...
    // particles 6
    {
        currentShader = drawParticles.cloud_.shader_;
        (*currentShader).bind();
        
        float mixfactor = .5;
        
        modelMx = drawParticles.cloud_.mx_;
        viewMx = w2cam;
        ...
        glUniformMatrix4fv(a = glGetUniformLocation((*currentShader).getId(), "mvpMx"), 1, GL_FALSE, &mvpMx[0]);
        glUniformMatrix3fv(a = glGetUniformLocation((*currentShader).getId(),"normalMx"), 1, 0, &normalMx[0]);
        glUniform1f(a = glGetUniformLocation((*currentShader).getId(),"pointSize"), particleRadius*scaleTarget);
        glUniform1f(a = glGetUniformLocation((*currentShader).getId(),"mixfactor"), mixfactor);
        
        glUniform1i(a = glGetUniformLocation((*currentShader).getId(), "text0"), 0);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_CUBE_MAP, drawParticles.cloud_.textId_[DrawObject::eText0]);
...
//    specCubeFiles.push_back(GetBundleResourcePath("desert_px.png"));
//    specCubeFiles.push_back(GetBundleResourcePath("desert_nx.png"));
//    specCubeFiles.push_back(GetBundleResourcePath("desert_py.png")); 
//    specCubeFiles.push_back(GetBundleResourcePath("desert_ny.png")); 
//    specCubeFiles.push_back(GetBundleResourcePath("desert_pz.png")); 
//    specCubeFiles.push_back(GetBundleResourcePath("desert_nz.png")); 
...
//    diffCubeFiles.push_back(GetBundleResourcePath("desert_d_px.png"));
//    diffCubeFiles.push_back(GetBundleResourcePath("desert_d_nx.png"));
//    diffCubeFiles.push_back(GetBundleResourcePath("desert_d_py.png")); 
//    diffCubeFiles.push_back(GetBundleResourcePath("desert_d_ny.png")); 
//    diffCubeFiles.push_back(GetBundleResourcePath("desert_d_pz.png")); 
//    diffCubeFiles.push_back(GetBundleResourcePath("desert_d_nz.png")); 
...
//        draw.shader_ = &cubemapShader;
    ...
    // particles setup
    {
        ...
        drawParticles.init(particles, particleShader, true);
        drawParticles.cloud_.textId_[DrawObject::eText0] = cubeTexture[eDiff];
        SetCol(drawParticles.cloud_.mx_,3,trans);
    }
