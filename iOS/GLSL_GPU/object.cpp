...
void initObjectFromRawData(Object& obj, float verts[][3], unsigned vsize, unsigned faces[][3], unsigned fsize) {
    ...
    for (unsigned i=0;i<vsize;i++) {
        obj.vx_.push_back(Object::type_vx(verts[i][0],verts[i][1],verts[i][2]));
    }
    obj.fc_.reserve(fsize);
    for (unsigned i=0;i<fsize;++i) {
        obj.fc_.push_back(Object::type_fc(faces[i][0],faces[i][1],faces[i][2]));
    }
    obj.vn_.reserve(vsize);
    for (unsigned i = 0; i < vsize; i++) {
        obj.vn_[i]+=Object::type_vn(0.);
    }
}
...
//        std::cout<<obj.vx_[obj.vx_.size()-1]<<obj.vn_[obj.vn_.size()-1]<<std::endl;
    ...
    if (data.st_.size()) {
        ...
        while(itr!=uvm.end()) {
            ...
            //std::cout<<obj.vx_[obj.vx_.size()-1]<<obj.uv_[obj.uv_.size()-1]<<obj.vn_[obj.vn_.size()-1]<<std::endl;
            itr++;
        }
    }
    for (unsigned i=0;i<data.fc_.size();i+=3) {
        ...
        if (itr0!=uvm.end() && itr1!=uvm.end() && itr2!=uvm.end()) {
            ...
            obj.fc_.push_back(Object::type_fc((*itr0).setpos_,(*itr1).setpos_,(*itr2).setpos_));
...
void Object::calculateNormals() {
    ...
    for (unsigned i = 0; i < fsize; i++) {
        const Object::type_fc& face=fc_[i];
        Real3 a = vx_[face[0]];
        Real3 b = vx_[face[1]];
        Real3 c = vx_[face[2]];
...
void Object::calculateTangents() {
    ...
    for (unsigned i = 0;i<fc_.size(); ++i) {
        ...
        Real3 t = r* Real3((st2[1] * q1[0] - st1[1] * q2[0]) , (st2[1] * q1[1] - st1[1] * q2[1]) , (st2[0] * q1[2] - st1[1] * q2[2]));
        Real3 b = r* Real3((st1[0] * q2[0] - st2[0] * q1[0]) , (st1[0] * q2[1] - st2[0] * q1[1]) , (st1[0] * q2[2] - st2[0] * q1[2]));
