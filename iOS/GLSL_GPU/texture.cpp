...
void SetGLTexturePixels( const Image& img, const GLenum target) {
    GLenum format = 
        img.getChannels()==1? GL_LUMINANCE:
        img.getChannels()==3? GL_RGB:GL_RGBA;
    glTexImage2D(target, 0, format, img.getWidth(), img.getHeight(), 0, format, GL_UNSIGNED_BYTE, img.getBits());
    glGenerateMipmap(target);
}

void SetGLTexturePixels( const Texture<unsigned char>& text, const GLenum target)
{
    GLenum format = 
        text.getChannels()==1? GL_LUMINANCE:
        text.getChannels()==3? GL_RGB:GL_RGBA;
	glTexImage2D(target, 0, format, text.getWidth(), text.getHeight(), 0, format, GL_UNSIGNED_BYTE, &text[0]);
    glGenerateMipmap(target);
}
...
GLuint makeGL2DTexture(const char* file) {
    ...
    SetGLTexturePixels(img,GL_TEXTURE_2D);
    return ret;
}
...
GLuint makeGL2DTexture(const Texture<unsigned char>& text) {
    GLuint ret = 0;
    ret = CreateGLTexture(GL_TEXTURE_2D);
    SetGLTexturePixels(text, GL_TEXTURE_2D);
    return ret;
}
